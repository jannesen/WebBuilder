...
async function runAsync(build, config) {
    ...
    const newStates = [];
    await build.parallelAsync(build.fileitems(config, (targetname) => {
        ...
        try {
            ...
            if (result_sass.sourceMap) {
                ...
                await $util.writeTextFileAsync(item.dstfilename + ".map", JSON.stringify({
                    ...
                    sources: map.sources.map((f) => (f.startsWith('file:///') ? build.sourcemap_map($util.path_join(process.cwd(), f.substr(8))) : f)),
